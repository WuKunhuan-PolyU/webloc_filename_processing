import os, json, requests, toml, plistlib, time, sys, argparse, traceback, time
...
def simplify_filenames(filepaths: List[str], api_key: str, batch_size: int = 5) -> Dict[str, str]:
    ...
    for i in range(0, len(filepaths), batch_size): 
        ...
        print (f"BATCH {i//batch_size + 1}/{len(filepaths)//batch_size + 1} ...", end = "")
        time.sleep(1)
        ...
        for filepath in batch: 
            ...
            filename_list.append((filepath, prefix))
        ...
        try: 
            ...
            for i, simplified_name in enumerate(simplified_batch): 
                ...
                filepath = filename_list[i][0]
                prefix = filename_list[i][1]
...
def main():
    ...
    # Rename files
    rename_webloc_files(simplified_names, args.dry_run)
